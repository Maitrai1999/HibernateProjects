...
public class Main {
	...
	/*	Query query = ses.createQuery("delete Emp1 where eid=:id");
		query.setInteger("id", 4);
		int result=query.executeUpdate();
		System.out.println("emp1 deleted"+result); */
		
	/*	Query query = ses.createQuery("select count(eid) from Emp1");
		Iterator count = query.iterate();
		System.out.println("No.of rows :"+count.next()); */
		
		/*Query query = ses.createQuery("Select max(esal) from Emp1");
		Iterator count = query.iterate();
		System.out.println("Maximum id: "+count.next()); */
		
	/*	Query query = ses.createQuery("from Emp1 where esal between 200000 and 300000 ");
		List<Emp1> list = query.list();
		for(Emp1 ep:list)
		{
			System.out.println(" ename: " + ep.getEname() + "  " );
		} */
		
	/*	Query query = ses.createQuery("from Emp1 order by ename asc ");
		List<Emp1> list = query.list();
		for(Emp1 ep:list)
		{
